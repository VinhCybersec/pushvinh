...
const SignUpForm = ({ setSign }) => {
   const schema = yup.object({
     email: yup.string().required("Vui lòng nhập vào ô này").max(40).email(),
     password: yup.string().required("Vui lòng nhập vào ô này").max(30).min(7, "Độ dài tối thiểu 7 ký tự"),
     passwordCheck: yup.string().required("Vui lòng nhập vào ô này").oneOf([yup.ref("password"), null], "Không khớp với mật khẩu"),
     name: yup.string().required("Vui lòng nhập vào ô này").max(30).min(5),
   });
   const navigate = useNavigate();
   const dispatch = useDispatch();
